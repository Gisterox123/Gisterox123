...
优缺点：代码更加灵活，框架灵魂，可以在运行时分析和执行类中的方法
...
除了RunTimeException及其子类其他的都属于受检查异常
...
* 数据结构或对象<---->二进制字节流
* 序列化的目的在于网路传输或者持久化在内存和磁盘中
* java中不想序列化的字段使用"transient"关键字修饰

### transient:

* transient 只能修饰变量，不能修饰类和方法
* transient修饰的变量在反系列化后被指为默认值
* static 变量不属于任何对象无论有没有transient修饰均不会实例化
...
// 第二种方式
BufferedReader input = new  BufferedReader(new InputStreamReader(Sysyem.in));
...
* OutputStream（字节输出流）/Reader（字符输出流） 所有输出流的基类
